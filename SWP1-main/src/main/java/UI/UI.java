...
public class UI {
    ...
    public void printAppEmployees(ProjectsApp app) {
        ...
        public void printAppEmployeeswInt(ProjectsApp app) {
            System.out.println("");
            System.out.println("Listing all Employees in system");
            for (int i = 0; i < app.getEmployees().size(); i++) {
                System.out.println(i+1 + ". " + "Name: " + app.getEmployees().get(i).getName() + " -  Initials: " + app.getEmployees().get(i).getInitials());
            }
    }
            ...
            while (true) {
                ...
                switch (input) {
                    ...
                    case "3":
                    	System.out.println("Please provide admin password");
                        input = scanner.nextLine();
                        String password5 = input;
                        if (app.adminLogin(password5)==true) {
                        	  System.out.println("Admin login successfully");
                        	  ui.pause();
                        } else {
                        	 System.out.println("Wrong password");
                                            ...
                                            while (true) {
                                                ...
                                                switch (input) {
                                                    ...
                                                    case "1":
                                                        ...
                                                        if (selectedEmployee > 0 && selectedEmployee <= app.getEmployees().size()) {
                                                            if (app.projects.get(selection - 1).checkEmployeeList(app.getEmployees().get(selectedEmployee - 1))) {
                                                                System.out.println("This Employee is already added to this Activity. Returning");
                                                                ui.pause();
                                                                break;
                                                            } else {
                                                                app.projects.get(selection - 1).employees.add(app.getEmployees().get(selectedEmployee - 1));
                                                                System.out.println("Employee added to Activity");
                                                                ui.pause();
                                                            }
                                                        } else {
