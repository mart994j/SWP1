...
public class UI {
	...
	public void printActivityOptions(ProjectsApp app, int projectSelection, int activitySelection) {
		System.out.println("");
		System.out.println("Viewing Project: " + app.projects.get(projectSelection - 1).getProjectName());
		System.out.println("Viewing Activity: "
				+ app.projects.get(projectSelection - 1).activities.get(activitySelection - 1).getActivName());
		System.out.println("Please select an option");
		System.out.println("0. Back");
	...
	public void printActivityEmployees(ProjectsApp app, int selectionProject, int selectionActivity) {
		System.out.println("Listing all Employees in Activity: "
				+ app.projects.get(selectionProject - 1).activities.get(selectionActivity - 1).getActivName());
		for (int i = 0; i < app.projects.get(selectionProject - 1).activities.get(selectionActivity - 1).employees
				.size(); i++) {
			System.out.println(i + 1 + ". "
					+ app.projects.get(selectionProject - 1).activities.get(selectionActivity - 1).employees.get(i)
							.getName());
		}
	}
		...
		if (app.userLogin(input)) {
			...
			while (true) {
				...
				case "4":
					...
					// it is necessary to parse to int in order to control that the user input is
					// not out of bounds with the projects list
					int selection = Integer.parseInt(input);
					if (selection > 0 && selection <= app.projects.size()) {
						// Project menu
						while (true) {
							...
							case "1":
								...
								app.projects.get(selection - 1)
										.addActivity(new Activity(name, startWeek, endWeek, hourBudget));
								break;
							case "2":
								...
								if (selectActivity > 0
										&& selectActivity <= app.projects.get(selection - 1).activities.size()) {
									// Activity menu
									while (true) {
										...
										case "1":
											...
											if (app.projects.get(selection - 1).activities.get(selectActivity - 1)
													.checkEmployeeList(app.getEmployees().get(selectedEmployee - 1))) {
												System.out.println(
														"This Employee is already added to this activity. Returning");
												ui.pause();
												break;
											}
											app.projects.get(selection - 1).activities.get(selectActivity - 1).employees
													.add(app.getEmployees().get(selectedEmployee - 1));
							...
							case "4":
								...
								if (selectEmployee > 0 && selectEmployee <= app.getEmployees().size()) {
									if (app.projects.get(selection - 1)
											.checkEmployeeList(app.getEmployees().get(selectEmployee - 1))) {
										System.out.println("This Employee is already added to this Project. Returning");
										ui.pause();
										break;
									} else {
										app.projects.get(selection - 1).employees
												.add(app.getEmployees().get(selectEmployee - 1));
							...
							case "5":
								...
								for (int i = 0; i < app.projects.get(selection - 1).activities.size(); i++) {
									...
									hoursBudget += app.projects.get(selection - 1).activities.get(i)
											.getExpectedAmountOfHours();
								}
								System.out.println("");
								System.out.println(
										"Time data for Project: " + app.projects.get(selection - 1).getProjectName());
							...
							case "6":
								...
								if (selectProjectLeader > 0 && selectProjectLeader <= app.getEmployees().size()) {
									if (!app.projects.get(selection - 1)
											.checkEmployeeList(app.getEmployees().get(selectProjectLeader - 1))) {
										app.projects.get(selection - 1).employees
												.add(app.getEmployees().get(selectProjectLeader - 1));
									}
									app.projects.get(selection - 1)
											.setProjectLeader(app.getEmployees().get(selectProjectLeader - 1));
								} else {
