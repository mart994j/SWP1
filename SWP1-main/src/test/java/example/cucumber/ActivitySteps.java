...
public class ActivitySteps {
    ...
    public void thatAProjectExistAndHasAnActivity() {
        ...
        projectApp.getProjectWithName("Project1").addActivity(activity);
        if(projectApp.projects.size()>0 && projectApp.getProjectWithName("Project1").activities.size()>0){
            assertTrue(true);
        } else{
            assertFalse(false);
        }
    ...
    public void that_the_project_leader_is_logged_in () throws OperationNotAllowedException {
		ProjectApp projectApp = projectAppHolder.getProjectsApp();
    	projectApp.addEmployee(new Employee("Martin","MJ"));
        //assertTrue(true);
    ...
    public void addEmployeeWithTheNameInitalAndAgeToTheExisitingActivity(String string, String string2) {
		...
		ProjectApp projectApp = projectAppHolder.getProjectsApp();
        projectApp.getActivityWithName("Activity1","Project1").assignEmp(employee);
    }
}
